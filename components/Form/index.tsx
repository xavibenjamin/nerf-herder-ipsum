    ...
    <div className={`max-w-2xl w-full`}>
      <div className={`flex items-end gap-5 my-8`}>
        <div className={` rounded-lg relative bg-transparent`}>
          <label htmlFor="custom-input-number" className="w-full text-violet-100 text-md mb-3 block">Number of Paragraphs</label>
        ...
        <div>
          <button 
            className={`flex items-center gap-2 h-14 bg-violet-400 text-slate-950 active:scale-90 font-bold text-sm px-6 py-3 rounded`}
        ...
        {paragraphs.length > 0 && (
          <button 
            className={`flex items-center gap-2 h-14 relative text-violet-300 active:scale-90 font-bold text-sm px-6 py-3 border-2 border-violet-400 rounded`}
      ...
      {paragraphs.length > 0 && (  
        <div className={'bg-slate-950 p-6 rounded'}>
          {paragraphs.map((paragraph, index) => (
            <p className={`mb-2 text-slate-300`} key={index}>{paragraph}</p>
          ))}
        </div>
      )}
    </div>
